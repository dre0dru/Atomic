...
Create a static partial class and mark it with `[EntityExtensionsAPI]`. Declare the character data as `ValueKey<>` fields:
...
See the [Entity API Generator](../Entities/CodeGen/EntityExtensionsAPIAttribute.md) for all supported key types and
...
- Declares type-safe keys with `[EntityExtensionsAPI]`
