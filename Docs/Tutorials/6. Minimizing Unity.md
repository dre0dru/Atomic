...
To display all entities in the scene, the [EntityWorldView](../Entities/UI/EntityWorldView.md) class is used.
It monitors the state of the world and creates corresponding `EntityView` instances for any entities that are added.

<img width="600" alt="Interaction between EntityWorld and EntityWorldView" src="../Images/EntityWorldScheme.png" />


From the diagram, you can see that `EntityWorldView` takes a collection of entities as input and manages their
visual representation. It has a `Deactivate` method that disables the current collection. When entities are added to or
removed from the collection or the `EntityWorld`, `EntityWorldView` automatically spawns or despawns the corresponding
`EntityView` objects in the Unity scene.
