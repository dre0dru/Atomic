...
{
    /// <summary>
    /// Provides code fixes for <see cref="EventAPIDiagnostics.MissingInitializer"/> and
    /// <see cref="EventAPIDiagnostics.ParameterlessConstructor"/>.
    /// Inserts or replaces the initializer with <c>new(nameof(FieldName))</c>.
    /// </summary>
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(EventAPICodeFixProvider)), Shared]
    public sealed class EventAPICodeFixProvider : CodeFixProvider
    {
        public sealed override ImmutableArray<string> FixableDiagnosticIds
            => ImmutableArray.Create(
                EventAPIDiagnostics.MissingInitializerId,
                EventAPIDiagnostics.ParameterlessConstructorId);
            ...
            foreach (var diagnostic in context.Diagnostics)
            {
                ...
                context.RegisterCodeFix(
                    CodeAction.Create(
                        ...
                        equivalenceKey: nameof(EventAPICodeFixProvider)),
                    diagnostic);
            }
        }
